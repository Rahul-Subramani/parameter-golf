"""
Submission: Beat SOTA 1.1194 BPB
Built on PR #549 (LeakyReLU² + Legal TTT + Parallel Muon) stack.
Key improvements over SOTA:
- AdamW for TTT (better convergence than SGD)
- 4 TTT epochs (was 3)
- MTP auxiliary loss during training (better representations)
- BigramHash(2048) (was 1536)
- VE on layers 7,8,9,10 (was 9,10)
- Cosine warmdown (was linear)
- Warmdown 4000 (was 3500)
- Mixed int5(MLP)/int6(attn) quantization
- Magnitude pruning + lzma-9 compression
...
class Hyperparameters:
    ...
    # MTP: Multi-token prediction auxiliary loss (improves representations)
    ...
    # TTT: Legal score-first TTT with AdamW + per-layer LR
    # NOTE: PR #672's 30-epoch global TTT is ILLEGAL (trains before scoring).
    # We use legal chunk-based TTT: score chunk -> train on scored chunk -> next.
    # 4 epochs per chunk with AdamW + per-layer LR (fits in ~550s eval budget).
    ...
    # Per-layer LR multipliers for TTT (MLP output 2x, MLP input 0.5x, rest 1x)
    ttt_mlp_proj_lr_mul = float(os.environ.get("TTT_MLP_PROJ_LR_MUL", 2.0))
    ttt_mlp_fc_lr_mul = float(os.environ.get("TTT_MLP_FC_LR_MUL", 0.5))
    # Depth recurrence: repeat these layers for virtual depth (PR #686)
    # Set to empty string to disable (faster eval for TTT budget)
    recur_layers = os.environ.get("RECUR_LAYERS", "")
    # CROWN-Q: curvature-regularized quantization penalty during warmdown (PR #693)
    crownq_lambda = float(os.environ.get("CROWNQ_LAMBDA", 0.01))
    crownq_warmdown_only = bool(int(os.environ.get("CROWNQ_WARMDOWN_ONLY", "1")))
    # SWA/EMA blend ratio (PR #693: 0.5 = 50/50 blend)
    swa_ema_blend = float(os.environ.get("SWA_EMA_BLEND", 0.5))
    # Label smoothing for training
    label_smoothing = float(os.environ.get("LABEL_SMOOTHING", 0.01))
    # Magnitude pruning fraction before quantization
...
# --- Mixed int5/int6 quantization with GPTQ-lite clip search ---
...
def quantize_intN_per_row_gptqlite(t: Tensor, clip_range: int = 31) -> tuple[Tensor, Tensor]:
    """GPTQ-lite: per-row quantization with percentile clip search."""
    t32 = t.float()
    if t32.ndim == 2:
...
def mixed_quantize_int56(state_dict: dict[str, Tensor], mlp_clip: int = 15, attn_clip: int = 31):
    """Mixed int5(MLP)/int6(attn) with GPTQ-lite clip search."""
    ...
    for name, tensor in state_dict.items():
        ...
        if cat == "mlp" and t.ndim >= 1:
            q, s = quantize_intN_per_row_gptqlite(t, clip_range=mlp_clip)
            result[name + ".q"] = q
            result[name + ".scale"] = s
            meta[name] = {"type": "int5"}
        elif cat in ("attn",) and t.ndim >= 1:
            q, s = quantize_intN_per_row_gptqlite(t, clip_range=attn_clip)
            result[name + ".q"] = q
            result[name + ".scale"] = s
            meta[name] = {"type": "int6"}
        else:
...
def dequantize_mixed_int56(obj: dict, template_sd: dict[str, Tensor]) -> dict[str, Tensor]:
    result = obj["w"]
    meta = obj["m"]
    ...
    for name, orig in template_sd.items():
        ...
        if info in ("passthrough", "passthrough_ctrl"):
            ...
            if t.dtype == torch.float16 and orig_dtype in (torch.float32, torch.bfloat16):
                odtype = passthrough_orig_dtypes.get(name)
                t = t.to(getattr(torch, odtype) if odtype else orig_dtype)
            out[name] = t
            continue
...
class MLP(nn.Module):
    ...
    def forward(self, x: Tensor, up_w: Tensor, down_w: Tensor) -> Tensor:
        # LeakyReLU(0.5)² -- proven -0.003 BPB vs relu²
...
class GPT(nn.Module):
    def __init__(self, vocab_size: int, num_layers: int, model_dim: int, num_heads: int,
                 ...
                 label_smoothing: float = 0.0, recur_layers: str = ""):
        super().__init__()
        ...
        # Depth recurrence: virtual-to-physical layer mapping (PR #686)
        ...
        # Build virtual-to-physical layer mapping for depth recurrence
        if self.recur_layer_indices:
            cutoff = max(self.recur_layer_indices) + 1
            self.v2p = list(range(cutoff)) + self.recur_layer_indices + list(range(cutoff, num_layers))
        else:
            self.v2p = list(range(num_layers))
        self.num_virtual_layers = len(self.v2p)
        ...
        # Recreate skip_weights with correct size
        # (overrides the one created above -- it's fine, parameters are registered by name)
        ...
        # MTP heads: discarded at export, only used during training
    ...
    def _run_layers(self, x: Tensor, x0: Tensor, input_ids: Tensor) -> Tensor:
        """Forward through virtual layers using v2p mapping for depth recurrence."""
        n = self.num_layers
        skips: list[Tensor] = []
        ve_cache: dict = {}
        for vi in range(self.num_encoder_layers):
            pi = self.v2p[vi]  # physical block index
            ve = self._get_ve(pi, input_ids, ve_cache)
            x = self.blocks[pi](x, x0, self.qo_bank[pi], self.kv_bank[pi], self.kv_bank[n + pi],
                                self.qo_bank[n + pi], self.mlp_up_bank[pi], self.mlp_down_bank[pi], v_embed=ve)
            skips.append(x)
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        # MTP auxiliary loss (improves representations, discarded at export)
...
# --- Legal score-first TTT with AdamW (improved from SOTA's SGD) ---
...
def eval_val_sliding_ttt(
    ...
    Key improvement: AdamW optimizer with cosine LR (better convergence than SGD)."""
    ...
    log0(f"ttt_sliding:start chunks={num_chunks} chunk_tokens={ttt_chunk} "
         ...
         f"ttt_lr={args.ttt_lr} ttt_epochs={args.ttt_epochs} optimizer=adamw "
    ...
    # Per-layer LR groups (PR #672 recipe)
    ...
    for name, p in base_model.named_parameters():
        ...
        if freeze:
            ...
            if "mlp.proj" in name or "mlp_down" in name:
                proj_params.append(p)
            elif "mlp.fc" in name or "mlp_up" in name:
                fc_params.append(p)
            else:
                other_params.append(p)
    ttt_params = proj_params + fc_params + other_params
    log0(f"ttt_sliding:params proj={sum(p.numel() for p in proj_params)} "
         f"fc={sum(p.numel() for p in fc_params)} other={sum(p.numel() for p in other_params)} "
         f"frozen={sum(p.numel() for p in base_model.parameters() if not p.requires_grad)}")
    # AdamW with per-layer LR groups (PR #672: proj 3x, fc 0.5x, rest 1x)
    ttt_param_groups = [
        {"params": proj_params, "lr": args.ttt_lr * args.ttt_mlp_proj_lr_mul, "initial_lr": args.ttt_lr * args.ttt_mlp_proj_lr_mul},
        {"params": fc_params, "lr": args.ttt_lr * args.ttt_mlp_fc_lr_mul, "initial_lr": args.ttt_lr * args.ttt_mlp_fc_lr_mul},
    ...
    for ci in range(num_chunks):
        ...
        # Phase 1: SCORE this chunk (inference_mode = hard guarantee no weight mutation)
        ...
        with torch.inference_mode():
            for bi in range(0, len(my_windows), batch_seqs):
                ...
                for i, ws in enumerate(batch_ws):
                    ...
                    tb += (has_leading_space_lut[tgt] & ~is_boundary_token_lut[prev]).to(torch.float64)
                    byte_count += tb.sum()
        # Phase 2: TRAIN on this chunk (legal: already scored under inference_mode)
        is_last_chunk = (ci == num_chunks - 1)
        if not is_last_chunk and args.ttt_epochs > 0:
            base_model.train()
            chunk_seqs = (chunk_end - chunk_start) // seq_len
            if chunk_seqs > 0:
                # Cosine LR decays across chunks (aggressive early, gentle late)
                cos_lr_mul = 0.5 * (1.0 + math.cos(math.pi * ci / max(num_chunks - 1, 1)))
                for pg in optimizer.param_groups:
                    pg['lr'] = pg['initial_lr'] * cos_lr_mul
...
def main() -> None:
    ...
    # MTP head params go to Adam
    mtp_head_params = list(base_model.mtp_heads.parameters())
    if mtp_head_params:
        scalar_params.extend(mtp_head_params)
    ...
    log0(f"model_params:{n_params} (mtp_heads:{mtp_params_count})")
    xsa_layers = [i for i, b in enumerate(base_model.blocks) if b.attn.use_xsa]
    log0(f"XSA:last_{args.xsa_last_n} active:{xsa_layers}")
    log0(f"MTP:heads={args.mtp_num_heads} weight={args.mtp_loss_weight}")
    log0(f"VE:layers={args.ve_layers} BigramHash:{args.bigram_vocab_size}")
    log0(f"world_size:{world_size} grad_accum_steps:{grad_accum_steps}")
    log0(f"matrix_lr:{args.matrix_lr} warmdown:{args.warmdown_iters} label_smoothing:{args.label_smoothing}")
    ...
    def lr_mul(step: int, elapsed_ms: float) -> float:
        """Cosine warmdown schedule (smoother than linear, better final loss)."""
    ...
    while True:
        ...
        for micro_step in range(grad_accum_steps):
            ...
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                loss = model(x, y)
                # CROWN-Q: curvature-regularized quantization penalty (PR #693)
                if args.crownq_lambda > 0 and (not args.crownq_warmdown_only or scale < 1.0):
                    crownq_pen = torch.zeros((), device=device)
                    for bank in [base_model.qo_bank, base_model.kv_bank,
                                 base_model.mlp_up_bank, base_model.mlp_down_bank]:
                        w = bank.view(-1, bank.shape[-1])
                        row_max = w.abs().amax(dim=1).clamp(min=1e-10)
                        delta = row_max / 15.0  # intentionally oversized (PR #693)
                        quant_var = (delta ** 2) / 12.0
                        h_proxy = (w ** 2).mean(dim=1)
                        crownq_pen = crownq_pen + (h_proxy * quant_var).sum()
    ...
    # Apply EMA/SWA blend (PR #693: 50/50 blend for better generalization + quantization)
    ...
    if swa_state is not None and swa_count > 0:
        ...
        avg_state = {name: blend * swa_avg[name] + (1.0 - blend) * ema_avg[name] for name in current_state}
        log0(f"weight_avg:EMA+SWA blend={blend} swa_count={swa_count}")
    else:
        avg_state = {name: t.to(dtype=current_state[name].dtype) for name, t in ema_state.items()}
        log0("weight_avg:EMA only (no SWA snapshots collected)")
    ...
    # Export: exclude MTP heads (training-only)
    ...
    if args.prune_fraction > 0:
        ...
        log0(f"magnitude_pruning:fraction={args.prune_fraction}")

    # Unbank + mixed int5/int6 quantization + lzma compression
    export_sd_pruned = {k: v for k, v in base_model.state_dict().items() if "mtp_heads" not in k}
    sd_cpu = {k: v.detach().cpu() for k, v in export_sd_pruned.items()}
    unbanked_sd = _unbank_state_dict(sd_cpu, args.num_layers)
    quant_obj = mixed_quantize_int56(unbanked_sd, mlp_clip=15, attn_clip=31)
    ...
    if master_process:
        ...
        code_bytes = len(code.encode("utf-8"))
        log0(f"Serialized model int5/6+lzma: {quant_file_bytes} bytes")
        log0(f"Total submission size: {quant_file_bytes + code_bytes} bytes")
        if quant_file_bytes + code_bytes > 16_000_000:
            log0(f"WARNING: exceeds 16MB by {quant_file_bytes + code_bytes - 16_000_000} bytes!")
    ...
    deq_unbanked = dequantize_mixed_int56(quant_state, unbanked_sd)
    deq_state = _rebank_state_dict(deq_unbanked, args.num_layers, sd_cpu)

    # Build eval model (no MTP heads)
    eval_model = GPT(
        vocab_size=args.vocab_size, num_layers=args.num_layers, model_dim=args.model_dim,
        num_heads=args.num_heads, num_kv_heads=args.num_kv_heads, mlp_mult=args.mlp_mult,
        tie_embeddings=args.tie_embeddings, tied_embed_init_std=args.tied_embed_init_std,
    ...
    torch.cuda.synchronize()
    log0(f"final_int56_roundtrip val_loss:{q_val_loss:.4f} val_bpb:{q_val_bpb:.4f} "
         f"eval_time:{1000.0 * (time.perf_counter() - t_qeval):.0f}ms")
    log0(f"final_int56_roundtrip_exact val_loss:{q_val_loss:.8f} val_bpb:{q_val_bpb:.8f}")
    ...
    # Legal TTT (score-first with AdamW)
    if args.ttt_enabled:
        torch.cuda.synchronize()
